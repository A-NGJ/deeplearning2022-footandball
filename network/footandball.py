...
def get_active_cells(
    bbox_center_x, bbox_center_y, downsampling_factor, conf_width, conf_height, delta
):
    ...
    for idx, (boxes, labels) in enumerate(zip(bboxes, blabels)):
        ...
        for box, label in zip(boxes, labels):
            ...
            if label == BALL_LABEL:
                ...
                temp_x = (
                    torch.tensor(range(x1, x2 + 1)).float() * player_downsampling_factor
                    + (player_downsampling_factor - 1) / 2
                )
                temp_y = (
                    torch.tensor(range(y1, y2 + 1)).float() * player_downsampling_factor
                    + (player_downsampling_factor - 1) / 2
                )
...
class FootAndBall(nn.Module):
    ...
    def detect_from_map(
        self, confidence_map, downscale_factor, max_detections, bbox_map=None
    ):
        ...
        batch_size, h, w = (
            confidence_map.shape[0],
            confidence_map.shape[1],
            confidence_map.shape[2],
        )
        ...
        xc = indices % w
        xc = xc.float() * downscale_factor + (downscale_factor - 1.0) / 2.0

        yc = torch.div(indices, w, rounding_mode="trunc")
        yc = yc.float() * downscale_factor + (downscale_factor - 1.0) / 2.0
        ...
        if bbox_map is not None:
            ...
            bbox_map = torch.zeros((batch_size, 4, h * w), dtype=torch.float).to(
        ...
        detections = torch.zeros((batch_size, max_detections, 5), dtype=float).to(
            confidence_map.device
        )
    ...
    def detect(self, player_feature_map, player_bbox, ball_feature_map):
        ...
        ball_detections = self.detect_from_map(
            ball_feature_map, self.ball_downsampling_factor, self.max_ball_detections
        )
...
def build_footandball_detector1(
    ...
    # tensor with 2 channels downscaled 16 times is used for the player detection
    # (1 location corresponds to 16x16 pixel block)
