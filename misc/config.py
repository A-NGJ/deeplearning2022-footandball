...
class Params:
    ...
    def _check_params(self):
        assert os.path.exists(
            self.issia_path
        ), "Cannot access ISSIA CNR dataset: {}".format(self.issia_path)
        assert os.path.exists(
            self.spd_path
        ), "Cannot access SoccerPlayerDetection_bmvc17 dataset: {}".format(
            self.spd_path
        )
        for c in self.issia_train_cameras:
            assert (
                1 <= c <= 6
            ), "ISSIA CNR camera number must be between 1 and 6. Is: {}".format(c)
