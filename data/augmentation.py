...
denormalize_trans = transforms.Compose(
    [
        transforms.Normalize(
            mean=[0.0, 0.0, 0.0], std=[1.0 / e for e in NORMALIZATION_STD]
        ),
        transforms.Normalize(
            mean=[-e for e in NORMALIZATION_MEAN], std=[1.0, 1.0, 1.0]
        ),
    ]
)
...
def tensor2image(image, downscale=None):
    ...
    if downscale is not None:
        temp = cv2.resize(
            temp, (temp.shape[1] // downscale, temp.shape[0] // downscale)
        )
    return temp
...
class RandomAffine:
    ...
    def __call__(self, sample):
        ...
        inverse_affine_matrix[:2] = np.array(coeffs).reshape(
            2, 3
...
class NoAugmentation:
    def __init__(self, size):
        self.size = size
        self.augment = transforms.Compose(
            [CenterCrop(self.size), ToTensorAndNormalize()]
        )
