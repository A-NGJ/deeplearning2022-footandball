...
class IssiaDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path, cameras, transform, only_ball_frames=False):
        ...
        for camera_id in cameras:
            ...
            annotated_frames = [
                e for e in list(annotated_frames) if e > min_annotated_frame + 50
            ]
        ...
        self.no_ball_images_ndx = set(
            [ndx for ndx in range(self.n_images) if ndx not in self.ball_images_ndx]
        )
        print("ISSIA CNR: {} frames with the ball".format(len(self.ball_images_ndx)))
        print(
            "ISSIA CNR: {} frames without the ball".format(len(self.no_ball_images_ndx))
        )
    ...
    def get_annotations(self, camera_id, image_ndx):
        ...
        for (
            ...
            player_x,
            player_y,
        ) in self.gt_annotations[camera_id].persons[image_ndx]:
            boxes.append(
                (player_x, player_y, player_x + player_width, player_y + player_height)
            )
...
def create_issia_dataset(dataset_path, cameras, mode, only_ball_frames=False):
    ...
    dataset = IssiaDataset(
        dataset_path, cameras, transform, only_ball_frames=only_ball_frames
    )
    return dataset
