...
def _dist(x1, x2):
    ...
    return np.sqrt(
        (float(x1[0]) - float(x2[0])) ** 2 + (float(x1[1]) - float(x2[1])) ** 2
    )
        ...
        if e.attributes["name"].value == "BALL":
            ...
            for elem in ball_attributes:
                if elem.attributes["name"].value == "BallPos":
                    ...
                    assert False, (
                        "Unexpected attribute: " + elem.attributes["name"].value
                    )
...
def read_issia_ground_truth(camera_id, dataset_path):
    ...
    annotation_file = (
        "Film Role-0 ID-" + str(camera_id) + " T-0 m00s00-026-m00s01-020.xgtf"
    )
...
def evaluate_ball_detection_results(annotations, gt_annotations, tolerance):
    ...
    percent_correctly_classified_frames = sum([c for (_, _, c) in frame_stats]) / len(
        frame_stats
    )
...
def visualize_detection_results(
    camera_id, dataset_path, gt_annotations=None, annotations=None
):
    """
    ...
    while sequence.isOpened():
        ...
        if not gt_annotations is None:
            frame = _annotate_frame(
                frame, count_frames, gt_annotations, color=(0, 0, 255)
            )
...
if __name__ == "__main__":
    ...
    (
        avg_precision,
        avg_recall,
        percent_correctly_classified_frames,
    ) = evaluate_ball_detection_results(
    ...
    print(
        "Percent of correctly classified frames = "
